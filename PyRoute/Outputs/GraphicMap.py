...
class GraphicMap(MapOutput):
    ...
    def add_circle(self, center: Cursor, radius: int, line_width: int, fill: bool, scheme: Scheme):
        if self.colours[scheme] is None:
            return
        colour = self._get_colour(self.colours[scheme])
        origin = (center.x - radius) * self.input_scale, (center.y - radius) * self.input_scale
        termin = (center.x + radius) * self.input_scale, (center.y + radius) * self.input_scale
        ...
        #     band = radius + offset
        #     self.doc.ellipse([(center.x - band, center.y - band), (center.x + band, center.y + band)], outline=colour, width=)
