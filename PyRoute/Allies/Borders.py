    ...
    def _generate_borders(self, allyMap: AllyMap, enforce=True) -> None:
        ...
        for system in allyMap.keys():
            ...
            if self._set_border(allyMap, system, 2):  # down
                neighbor = Hex.get_neighbor(system, 2)
                self._set_border_colour(neighbor, 0, colours[0])
            ...
            if self._set_border(allyMap, system, 0):  # up right
                if system[0] & 1:
                    neighbor = Hex.get_neighbor(system, 1)
                    self._set_border_colour(neighbor, 1, colours[2])
                else:
                    neighbor = Hex.get_neighbor(system, 1)
                    self._set_border_colour(neighbor, 2, colours[3])

            if self._set_border(allyMap, system, 1):  # down right
                neighbor = Hex.get_neighbor(system, 1)
                if system[0] & 1:
                    self._set_border_colour(neighbor, 2, colours[4])
                else:
                    neighbor = Hex.get_neighbor(neighbor, 2)
                    self._set_border_colour(neighbor, 1, colours[5])

            if self._set_border(allyMap, system, 3):  # down left
                if system[0] & 1:
                    neighbor = Hex.get_neighbor(system, 2)
                    self._set_border_colour(neighbor, 2, colours[6])
                else:
                    neighbor = Hex.get_neighbor(system, 2)
                    self._set_border_colour(neighbor, 1, colours[7])

            if self._set_border(allyMap, system, 4):  # up left
                if system[0] & 1:
                    neighbor = Hex.get_neighbor(system, 2)
                    self._set_border_colour(neighbor, 1, colours[8])
                else:
